...
using ProgressMeter
using Random
export CVConstrainedOverdampedLangevin, euler_maruyama!, euler_maruyama_split_time!, PVD1!
...
@inline function euler_maruyama!(sys,
    ...
    P_F, gradφ = compute_Pvec_x(φ=sim.φ_grid, vec=accels_nounits_t, x=x_barₖ)
    ...
    for step_n in 1:n_steps
        ...
        compute_Pvec_x!(P_F, gradφ, φ=sim.φ_grid, vec=accels_nounits_t, x=x_barₖ)
        divP .= compute_divP(φ_flat=sim.φ_flat, gradφ=gradφ, x=x_barₖ)
        compute_Pvec!(Pnoise, gradφ=gradφ, vec=noise)
        x_barₖ .+= ((P_F + k_bT * divP) ./friction_nounits) .* dt .+ noise_scaling .* Pnoise   # no explicit division by m here because we assume an identity mass matrix (in units of g mol^-1)
...
@inline function euler_maruyama_split_time!(sys,
    ...
    for step_n in 1:n_steps
        ...
        divP .= compute_divP(φ_flat=sim.φ_flat, gradφ=gradφ, x=x_barₖ)
        compute_Pvec!(Pnoise, gradφ=gradφ, vec=noise)
        x_barₖ .+= ((P_F) ./friction_nounits) .* dt .+ noise_scaling .* Pnoise   # no explicit division by m here because we assume an identity mass matrix (in units of g mol^-1)
...
@inline function PVD1!(sys,
    ...
    noise = similar(sys.velocities)
    noise = sample_noise_vector(num_coords) # dimensionless 
    noise_scaling = sqrt(k_bT * dt / (2 * friction_nounits)) / (1^(-1/2))  # we assume an identity mass matrix (in units of g mol^-1)
    # @assert are_units_same(uX, sqrt(uk * uT * ut / uγ) / sqrt(uM))
    ...
    for step_n in 1:n_steps
        ...
        divP .= compute_divP(φ_flat=sim.φ_flat, gradφ=gradφ, x=x_barₖ)
        F_x_barₖ .= (P_F + k_bT .* divP) ./ friction_nounits
        # The grad_phi that is going into this function is grad_phi evaluated at x_bar_k
        x .+= dt * F_x_barₖ + 2 * noise_scaling * compute_Pvec_x(φ=sim.φ_grid, vec=noise, x=x + (1/4)* dt * F_x_barₖ₋₁)[1]
        
        # MT2!(x .+ dt * F_x_barₖ₋₁ / 4, dt, k_bT, friction_nounits, sim.φ_flat, noise)
