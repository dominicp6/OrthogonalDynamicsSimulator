...
@inline function PVD2_split_time!(sys, sim::CVConstrainedOverdampedLangevin, n_steps::Integer; n_threads=Threads.nthreads(), tol=1e-6, max_iter=50, run_loggers=true, compute_ergodic_integral=false, quantity=nothing)
    x, forces_nounits_t, accels_nounits_t, noise, P_F, Pnoise, divP, gradφ, laplacianφ, neighbors, forces_buffer, 
    masses_nounits, friction_nounits, k_bT, dt = init_simulation(sys, sim, n_threads, run_loggers, compute_ergodic_integral, quantity)
    ...
    for step_n in 1:n_steps
        ...
        λ, stopping_condition, error, num_iter = find_lambda(sim.φ_grid, x, divPₖ, k_bT, friction_nounits, dt, constrained_CV; tol=tol, max_iter=max_iter)
        stopping_condition_counts[stopping_condition] += 1
        errors[step_n] = error
        num_iterations[step_n] = num_iter
        @. x .+= (k_bT * divP / friction_nounits) * λ * dt
